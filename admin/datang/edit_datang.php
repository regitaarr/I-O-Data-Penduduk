<?php
    ...
    if(isset($_GET['kode'])){
        $sql_cek = "SELECT d.id_datang, d.nik, d.nama_datang, d.jekel, d.tgl_datang, p.nik, p.nama from 
		tb_datang d inner join tb_pdd p on d.pelapor=p.nik WHERE id_datang='".$_GET['kode']."'";
			...
			<div class="form-group row">
				...
				<div class="col-sm-6">
					<select name="pelapor" id="prlapor" class="form-control select2bs4" required>
                        ...
                        ?>
						<option value="<?php echo $row['nik'] ?>" <?=$data_cek[
						 'nik']==$row[ 'nik'] ? "selected" : null ?>>
							<?php echo $row['nik'] ?>
							-
							<?php echo $row['nama'] ?>
						</option>
